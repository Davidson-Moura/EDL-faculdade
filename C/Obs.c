...
static THREAD_LOCAL bool is_ui_thread = false; //Alocação de memória estática #$%-1
...
static int obs_init_video(struct obs_video_info *ovi)
{
	...
	int errorcode; //Declaração na Pilha #$%-8
...
{
	obs = bzalloc(sizeof(struct obs_core)); //Alocação na Heap #$%-6
...
bool obs_startup(const char *locale, const char *module_config_path,
		 profiler_name_store_t *store)
{
	bool success; //Declaração na Pilha #$%-5
...
void obs_set_cmdline_args(int argc, const char *const *argv) //Declaração na Pilha dos argc, *argv #$%-4
{
	char *data;
	size_t len;
	int i; //Declaração na Pilha #$%-3
	...
	cmdline_args.argv = bmalloc(sizeof(char *) * (argc + 1) + len); //Alocação na Heap #$%-2 
...
#undef FREE_REGISTERED_TYPES
	...
	bfree(obs->locale);
	bfree(obs);//Desalocação na Heap #$%-11
	obs = NULL;
...
#define OBS_SIZE_MIN 2 //Alocação de memória estática #$%-10
...
bool obs_enum_filter_types(size_t idx, const char **id) //Declaração na Pilha #$%-9
{
...
{
	context->control = bzalloc(sizeof(obs_weak_object_t)); //Alocação na Heap #$%-7
...
void obs_weak_object_release(obs_weak_object_t *weak)
{
	...
	if (obs_weak_ref_release(&weak->ref))
		bfree(weak); //Desalocação na Heap #$%-12
}
